...
def download_deps(mcp_path):
    ...
    for lib in ['argo-2.25.jar', 'guava-12.0.1.jar', 'guava-12.0.1-sources.jar', 'asm-all-4.0.jar', 'asm-all-4.0-source.jar', 'bcprov-jdk15on-147.jar']:
        ...
        if not os.path.isfile(target):
            try:
                urllib.urlretrieve('https://web.archive.org/web/20130627141007/http://files.minecraftforge.net/fmllibs/' + lib, target)
                print 'Downloaded %s' % lib
            except:
                print 'Download %s failed, download manually from https://web.archive.org/web/20130627141007/http://files.minecraftforge.net/fmllibs/%s or https://web.archive.org/web/20130627141007/http://files.minecraftforge.net/fmllibs/fml_libs_dev.zip and place in MCP/lib' % (lib, lib)
                ret = False
